...
{note}The SYCL definition of "inclusive scope" follows the OpenCL definition.
In OpenCL versions prior to 3.1, the memory scopes of _A_ and _B_ must be the
same for their scopes to be inclusive, but this was relaxed in OpenCL version
3.1.
As a result, implementations of SYCL 2020 using an OpenCL backend may use the
older OpenCL semantic, but implementations of {SYCL_VERSION} must use the OpenCL
3.1 semantic.{endnote}
